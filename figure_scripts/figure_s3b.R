...
figure_s3b() <- function(){
genelist1 <- read.csv("/users/andrew leader/google drive/merad/scRNAseq_analysis/results/AL/lung_main/inputs/chakarov_Table_S4.csv",stringsAsFactors=F,r=1,h=1)[,2]
genelist2 <- read.csv("/Users/Andrew Leader/Google Drive/merad/scRNAseq_analysis/results/AL/lung_main/inputs/chakarov_Table_S5.csv",stringsAsFactors=F,r=1,h=1)[,2]

cells <- lung_ldm$dataset$cell_to_cluster=="8"

setdiff(genelist2,rownames(lung_ldm$dataset$umitab))
...
cor.test(score1,score2,method="spearman")
