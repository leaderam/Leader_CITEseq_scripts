...
figure_2ab <- function(){
  ...
  genes <- strsplit(genes_in_figures["DC",],",")[[1]]
  markers <- strsplit(markers_in_figures["DC",],",")[[1]]
  cluster_order <- as.character(strsplit(cluster_order_list["cDC",],",")[[1]])
  lineage_col_array <- rgb(t(col2rgb(c(2,3,6,"orange")))*0.7/255)
  names(lineage_col_array) <- c("moDC","cDC2","mregDC","cDC1")
  ...
  output_file <- "output/figures/figure_2ab.png"
  height=4.54
  width=4.0
  res=1000
  pointsize=6
