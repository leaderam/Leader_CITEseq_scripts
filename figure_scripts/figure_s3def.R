...
# load human annots
annot_list <- read.csv(file.path(data_dir,"model_lung_190606_annot_lists.csv"),
...
for(mod_iter in names(mods)){
  xlim <- range(log10(10^-3.5+mod_scores[mods[mod_iter],annot_list[as.character(metadata[colnames(mod_scores),"cluster_assignment"]),"sub_lineage"]%in%annot_ord]),na.rm=T)
