...
clusts <- strsplit(cluster_order_list["Mono_mac_modules",],",")[[1]]
...
modules <- parse_modules_leader(ldm=lung_ldm,cormat = cmat,ds_version = "2000", modules_version = "Mono_mac",
                                      tables_path = "tables/",nmods = 50,min_mod_cor = 0.1)

#modules <- read.table("tables/fig_3_modules.txt",sep="\t",stringsAsFactors = F)$V1
modules <- read.table("tables/Mono_mac_modules.txt",sep="\t",stringsAsFactors=F)$V1
...
write.csv(t.res,"output/statistics/figure_s3g.csv")

png("output/figures/figure_s3efg.png",
...
# load human annots
annot_list <- read.csv("tables/annots_list.csv",
                       r=1,h=1,stringsAsFactors=F)
annot_ord <- rev(c("CD14 mono","AZU1_mac","MoMac-I","MoMac-II","MoMac-III","MoMac-IV","IM","AM"))

subtype_2_col <- rgb(t(col2rgb(c(6,"green",
                                 4,rgb(0,.5,1),rgb(0,.8,1),5,"red","yellow","orange"
))*0.7/255))
names(subtype_2_col) <- annot_ord

mods <- c("4","46","2","14","11","27","15")
names(mods) <- c("C1Q genes","HLA-class-II","Glycolysis","APOE-group","TREM2-group","IM-like","AM-like")


png("output/figures/figure_3c.png",height=2.75,width=12,units="in",res=300,bg="transparent")
layout(matrix(seq(length(mods)),nrow=1))
par(mgp=c(2,1,0),mar=c(3,1,2,1))
for(mod_iter in names(mods)){
  xlim <- range(log10(10^-3.5+mod_scores[mods[mod_iter],annot_list[as.character(lung_ldm$dataset$cell_to_cluster[colnames(mod_scores)]),"sub_lineage"]%in%annot_ord]),na.rm=T)
  plot(1:10,col="white",xlim=xlim,ylim=c(0,9),ylab="",yaxt="n",bty="L",xlab="")
  for(iter in rev(seq(length(annot_ord)))){
    #cell_mask <- annot_list[as.character(metadata[colnames(mod_scores),"cluster_assignment"]),"sub_lineage"]==annot_ord[iter]
    cell_mask <- annot_list[as.character(lung_ldm$dataset$cell_to_cluster[colnames(mod_scores)]),"sub_lineage"]==annot_ord[iter]
    d <- density(log10(10^-3.5+mod_scores[mods[mod_iter],cell_mask]))
    polygon(d$x,iter-1+d$y,col=subtype_2_col[iter])
    abline(h=iter-1)
    mtext(paste(mods[mod_iter],": ",mod_iter,sep=""))
  }
}
dev.off()
...
png("output/figures/figure_s3h.png",height=1.74,width=4.38,units="in",res=300,pointsize=5,bg="transparent")
