...
figure_4fg <- function(){
  ...
  genes <- strsplit(genes_in_figures["B_plasma",],",")[[1]]
  markers <- strsplit(markers_in_figures["B_plasma",],",")[[1]]
  cluster_order <- as.character(strsplit(cluster_order_list["B&Plasma",],",")[[1]])
  lineage_col_array <- c(rgb(t(col2rgb(c(2,3,4,5,6)))*0.7/255))
  ...
  output_file <- "output/figures/figure_4fg.png"
  height=4.32
  width=5.34
  res=1000
  pointsize=6
